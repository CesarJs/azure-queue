# Microsoft Azure Storage Queue PHP Client Library
...
```php
use MicrosoftAzureEtapa\Storage\Common\ServiceException;
```
...
`MicrosoftAzureEtapa\Storage\Queue\Models\ListQueuesOptions`.
...
implements `MicrosoftAzureEtapa\Storage\Common\Internal\IMiddleware`, or a
`callable` that follows the Guzzle middleware implementation convention.

User can create self-defined middleware that inherits from `MicrosoftAzureEtapa\Storage\Common\Internal\Middlewares\MiddlewareBase`.
